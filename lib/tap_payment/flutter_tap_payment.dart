...
class TapPaymentState extends State<TapPayment> {
  ...
  loadPayment() async {
    ...
    try {
      ...
      if (getPayment['error'] == false && getPayment['message']?['transaction']?["url"] != null) {
        setState(() {
  ...
  void initState() {
    ...
    services = TapServices(apiKey: widget.apiKey, paymentData: {...widget.paymentData, ...formData});
    setState(() {
          ...
          onNavigationRequest: (NavigationRequest request) {
            ...
            if (request.url.contains(widget.redirectUrl)) {
              ...
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(builder: (context) => CompletePayment(url: request.url, services: services, onSuccess: widget.onSuccess, onError: widget.onError)),
              );
            }
