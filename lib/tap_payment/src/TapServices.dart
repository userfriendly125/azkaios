...
class TapServices {
  ...
  sendPayment() async {
    ...
    try {
      var response = await http.post(Uri.parse(domain), body: jsonEncode(paymentData), headers: {
  ...
  confirmPayment(tapId) async {
    ...
    try {
      var response = await http.get(Uri.parse(domain), headers: {"content-type": "application/json", 'Authorization': 'Bearer $apiKey'});
